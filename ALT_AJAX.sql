...
DROP TABLE IF EXISTS `AJAX_CURR`.`device_group` ;

CREATE TABLE IF NOT EXISTS `AJAX_CURR`.`device_group` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;

INSERT INTO `AJAX_CURR`.`device_group` (`id`,`name`)
VALUES
  (1,"kitchen"),
  (2,"garage"),
  (3,"staff room"),
...
CREATE TABLE IF NOT EXISTS `ajax_curr`.`device` (
  ...
  `device_group_id` INT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_device_hub_idx` (`hub_id` ASC) VISIBLE,
  INDEX `fk_device_device_group1_idx` (`device_group_id` ASC) VISIBLE,
  CONSTRAINT `fk_device_device_group1_idx`
    FOREIGN KEY (`device_group_id`)
    REFERENCES `AJAX_CURR`.`device_group` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
INSERT INTO `AJAX_CURR`.`device` (`id`,`model`,`status`,`service_life_end_time`,`warranty_end_time`,`on_battery`,`hub_id`,`device_group_id`)
...
INSERT INTO `AJAX_CURR`.`users_group` (`id`,`name`,`description`,`hub_id`)
...
INSERT INTO `AJAX_CURR`.`permission` (`id`,`name`,`hub_id`,`user_id`,`group_id`,`device_id`)
VALUES
