...
CREATE TABLE IF NOT EXISTS `AJAX_CURR`.`hub` (
  ...
  `hub_status` VARCHAR(45) NULL DEFAULT 'offline',
...
INSERT INTO `AJAX_CURR`.`device` (`id`,`model`,`device_status`,`service_life_end_time`,`warranty_end_time`,`on_battery`,`hub_id`)
...
DROP TABLE IF EXISTS `AJAX_CURR`.`hub_group` ;

CREATE TABLE IF NOT EXISTS `AJAX_CURR`.`hub_group` (
  `id` INT NOT NULL AUTO_INCREMENT, 
  -- NNULL
  `group_name` VARCHAR(45) NULL DEFAULT 'undefined',
...
INSERT INTO `AJAX_CURR`.`hub_group` (`id`,`group_name`,`group_description`,`hub_id`)
...
CREATE TABLE IF NOT EXISTS `AJAX_CURR`.`user` (
  ...
  CONSTRAINT `fk_user_group1`
    ...
    REFERENCES `AJAX_CURR`.`hub_group` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

INSERT INTO `AJAX_CURR`.`user` (`id`,`email`,`user_password`,`date_created`,`user_name`,`group_id`)
VALUES
  ...
  (3,"at.iaculis@google.net","RSL21QLS2YI","2022-07-9 10:05:04","John",null),
...
CREATE TABLE IF NOT EXISTS `AJAX_CURR`.`permission` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `permission_name` VARCHAR(45) NOT NULL DEFAULT 'undefined',
  `permission_description` VARCHAR(45) NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;

INSERT INTO `AJAX_CURR`.`permission` (`id`,`permission_name`,`permission_description`)
VALUES
...
CREATE TABLE IF NOT EXISTS `AJAX_CURR`.`rex` (
  ...
  `rex_range` INT NOT NULL DEFAULT 500,
  `hub_id` INT NULL,
  PRIMARY KEY (`id`),
